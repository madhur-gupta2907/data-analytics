...
|---|---|---|
| **Member 1** | Project Lead | Streamlit routing, icon-safe CSS, dark/light theme |
| **Member 2** | Data Engineer | `load_and_clean_data` — bytes caching, auto date detection, Sales derivation, dedup |
| **Member 3** | Data Analyst | KPI cards, Data Quality Report, Plotly charts, EDA & Correlation module |
| **Member 4** | ML Engineer | LinearRegression with time-aware train/test split, hold-out R², MAE, residual chart |
...
| # | Bug | Fix Applied |
|---|---|---|
| 1 | Icon text overlap (`arrow_drop_down` rendered as raw text) | Scoped `font-family` to text elements only — never `* {{}}` |
| 2 | `@st.cache_data` crashed on `UploadedFile` object | Passes raw `bytes` instead |
| 3 | `width='stretch'` is not a valid Streamlit argument | Replaced with `width='stretch` |
| 4 | Dark mode didn't affect Plotly charts | `PLOTLY_TPL` switches per theme |
| 5 | ML R² was in-sample only (inflated/misleading) | 80/20 time-aware split + hold-out R² + MAE |
| 6 | Hard-coded column names crashed on non-Superstore data | Auto-detection with hint sets + sidebar override |
| 7 | Internal columns (Ordinal_Date, Year, Month) polluted pickers | Excluded via `INTERNAL_COLS` set |
| 8 | "3-Member workflow" counted 4 members | Corrected to **4-Member Team** |
    """)
